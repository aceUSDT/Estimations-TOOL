...
function masterPrompt({ textLines, primary, second, mismatches }) {
  return [
    ...
    'Also report authored source contradictions: compare printed labels with physical',
    'row/column geometry, this document\'s repeated patterns, board headers, legends,',
    'and electrical invariants. State both printed and inferred values; never silently',
    'repair a source or borrow a pattern from a different document.',
    '',
...
function validExtraction(value) {
  return Boolean(value && typeof value === 'object'
    ...
    && Array.isArray(value.boards) && Array.isArray(value.devices)
    && Array.isArray(value.feeds) && Array.isArray(value.flags));
}

async function readExtractionRole(pool, role, request, initialExclude = []) {
  const excluded = new Set(initialExclude);
  let lastError = null;
  for (let attempt = 0; attempt < 12; attempt += 1) {
    let response;
    try {
      response = await pool.callRole(role, request, { exclude: [...excluded] });
    } catch (error) {
      lastError = error;
      break;
    }
    const parsed = parseModelJson(response.content);
    if (validExtraction(parsed)) return { response, result: coerceResult(parsed) };
    excluded.add(response.model);
    lastError = Object.assign(new Error(`Invalid structured extraction from ${response.model}`), {
      code: 'invalid_extraction_json',
    });
  }
  const error = lastError || new Error(`No valid ${role} extraction was returned`);
  if (!error.code || error.code === 'invalid_extraction_json') error.code = 'role_exhausted';
  throw error;
}
...
export async function runAgentTeam(page, deps) {
  ...
  try {
    const secondRead = await readExtractionRole(deps.pool, 'second_opinion', req, [a.model]);
    b = secondRead.response;
    second = secondRead.result;
  } catch (e) {
