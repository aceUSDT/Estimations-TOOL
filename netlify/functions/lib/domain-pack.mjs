...
- trimble_cable (Trimble/SVA "Cable Schedule"): the board identity may exist only as the repeated prefix of a hierarchical left-column Id No such as FF-L&P-3/L/1-L1. Interpret that example as board FF-L&P-3, lighting way L1, phase L1; /L/1-L2 and /L/1-L3 are the other phase rows of the same way, /L/2-* is lighting way L2, and /P/1-* is power way P1. Bind the right-hand Ir(A), In(A), Type, RCD and AFDD cells to the same bounded identifier row. Connected From/Connected To values are source or circuit references, not additional schedule boards.
...
- transposed: a schedule embedded in a drawing where field names form horizontal bands (for example CPC / CABLE / MCB / PHASE / AFDD / TYPE / WAY) and each outgoing circuit occupies a vertical page column. Orientation is irrelevant: bind values by the intersecting field band and circuit column, then return one ordinary device row per printed WAY. A board title such as "DISTRIBUTION BOARD REF 1 GARAGE" or "MAIN MCCB BOARD M1 GARAGE" is the identity for that local matrix.
...
1. READ IN THIS ORDER: (a) board identity/header and incomer, (b) every way/phase slot and its protective device, rating, curve, RCD/AFDD, poles and breaking capacity, (c) associated equipment, (d) supply relationships, and only then (e) cable details. Cable fields must never displace or substitute for protection-device fields. In UK protection columns, BS EN 60898 identifies an MCB, BS EN 61009 identifies an RCBO, BS EN 61008 identifies an RCD/RCCB, BS EN 60947-2 identifies an MCCB, and BS EN 60947-3 identifies a switch-disconnector/isolator. After the BS standard, read the adjacent Type/Curve, Rating (A), short-circuit capacity (kA), AFDD, RCD and RCD operating-current (mA) cells in that exact column order. Never take a later live-conductor, CPC, cable-size or voltage-drop number as the protective-device rating.
   ...
   - Commercial grouping rule: an outgoing, non-incomer MCB row with explicit 30mA residual protection belongs in the RCBO procurement group, including when the drawing depicts the 30mA RCD separately. Return device_class="RCBO", retain rcd_arrangement="separate"/"shared"/"upstream" exactly as evidenced, and describe the printed MCB class in the flag/evidence trail. Do not apply this conversion when 30mA is absent or the row is an incomer.
...
8. Schematics: treat the drawing as a graph. Follow drawn conductors from source → main panel → outgoing device → cable → downstream board; NEVER infer a feed from proximity, vertical order, or matching names. A line crossing without a junction dot is not a connection. Emit every downstream board node and every resolved feed edge with its protective device and cable (ref, csa, cpc, type). Include SPDs, EVC pillars, lifts/ATS, generators, UPS as boards/devices with their annotations in description. Unresolved endpoints stay unresolved and are flagged.
   - First partition each equipment block into: board identity/details, incomer, busbar, outgoing ways/devices, connected loads or sub-boards, and accessories such as meters/SPDs. Panelboard blocks may contain a compact or transposed schedule; extract their ways with the same row/column rules as a standalone DB schedule.
   - Short labelled identities are authoritative schematic nodes: "REF: MSP1", "REF: PBT1", "REF: PBLL1", "REF: LS1", "REF: DBT4", and "REF: DB ESS" are board references exactly as printed. Do not truncate PBT1 to PB1. Labels such as "LL PB 1" or "LS PB 2" beside an outgoing way are circuit/load labels, not independent board identities unless a separate equipment block explicitly identifies them.
   - For each board block, keep panel rating/fault current/phase configuration, incoming device and rating, every outgoing device and rating, poles/trip unit, cable, meter, SPD, spare and downstream reference in their own fields. "MCCB-160A 3-N 160A" is an outgoing MCCB record; a nearby panel rating is not that outgoing rating.
   - If a conductor exits the current board block, continue following it upstream or downstream across the entire drawing before assigning from_ref/to_ref. Do not stop at the nearest text block, and do not substitute page order for a conductor path.
   - Respect the PDF/image orientation and any page rotation. Read labels in their displayed orientation, but use the same source coordinates for every board, device and path so evidence remains selectable after rotation.
...
16. A tick, cross, Yes, No, or sensitivity in an RCD/RCBO column belongs to the circuit row whose horizontal band contains that mark. Never detach a protection mark from its row or borrow one from an adjacent row.
   - In cable-schedule protection columns, N/A means that protection does not apply to that row. MCB + RCD N/A + AFDD N/A remains an MCB with rcd_protected=false and afdd=false; never relabel it RCBO.
   - When a trip curve is omitted, infer C only when the same board's printed/calibrated rating is 100A through 250A and the device family supports a curve. Record the inference and require human review. Above 250A, or when board rating is absent, leave curve blank.
  ...
  properties: {
    classification: {
      ...
      properties: {
        ...
        sub_format: { type: 'string', enum: ['amtech', 'trimble', 'bes', 'bam_epo', 'syntegral', 'hevacomp', 'imsc_tba', 'cu', 'switchboard', 'mccb', 'hager_grid', 'transposed', 'simple', 'unknown'] },
        confidence: NUM,
      },
    },
