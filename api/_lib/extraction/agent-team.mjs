...
function boardContract(facts, role) {
  ...
  if (facts && facts.schematic) {
    lines.push(
      ...
      '22. A rating written as a PAIR is a frame size and a trip setting. Return',
      '    both rather than choosing one. It appears with a separator or STACKED,',
      '    one figure above the other:',
      '      100/125 A          63A        40A       100A',
      '                         100A       100A      100A',
      '    where the upper figure is the SETTING and the lower the FRAME, and a way',
      '    set to its full frame prints the same number twice. Two numbers above one',
      '    another on a way are never two devices.',
      ...
      '30. GREYING IS A CONVENTION, not decoration. Where most items are drawn in',
      '    colour or black and a few are washed out in grey, the grey ones are',
      '    existing, future or by others — and the drawing frequently says so',
      '    NOWHERE in words. Return them like any other item with a flag recording',
      '    that they are drawn greyed. Dropping them silently is one failure;',
      '    pricing them as new is the expensive one.',
      '31. The label above or beside an outgoing way names WHAT IT FEEDS — a',
      '    sub-board reference and its duty ("DB/LM/1/2 LIGHTING & MECHANICAL",',
      '    "DB/P/1/KIT KITCHEN", "LIFT", "FAP"). That is the feed relationship for',
      '    that way, drawn as a caption rather than as a line into a box. Return the',
      '    way and the board it feeds together.',
      '32. Cable sizes are often listed ONCE at the side of the drawing ("10mm2',
      '    XLPE/SWA/LSF + 6mm2 SWA CPC" and two others) and tied to ways only by',
      '    which line a way touches, or by a shading or colour group. Do NOT attach',
      '    a cable size to a way unless the drawing makes that link explicit.',
      '    Return the sizes as the BOARD\'s, and say in "layout" that the per-way',
      '    mapping was not legible when it was not.',
      '33. A busbar has ratings of its own — "BUSBAR RATING: 250 A", "BUSBAR SHORT',
      '    CIRCUIT WITHSTAND: 36 kA 1 SEC" — and a construction form ("FORM 3b,',
      '    TYPE 2", "FORM 4 TYPE 2"). They belong to the board, not to any way.',
    );
  }
