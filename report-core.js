(function (root, factory) {
  ...
  function includeRow(row) {
    if (!row || row.status === "rejected" || row.outOfScope || !row.device) return false;
    if (row.kind === "schematic" || String(row.sourceRole || "").startsWith("schematic_")) return false;
    if ((row.kind === "mention" || row.kind === "ai") && row.status !== "confirmed") return false;
    return Math.max(1, Number(row.qty) || 1) > 0;
  }
  ...
  function contributorFor(row, specification, board, fileNames) {
    ...
    return {
      ...
      earthFaultDevice: text(row && (row.earthFaultDevice?.descriptor || row.earthFaultDevice)) || null,
      arcFlashDevice: text(row && row.arcFlashDevice) || null,
      circuitConfig: text(row && row.circuitConfig) || null,
      serviceCode: text(row && row.serviceCode) || null,
      discipline: text(row && row.discipline) || null,
      cableDetails: text(row && (row.cable?.orig || row.cable)) || null,
      sourceDocument,
