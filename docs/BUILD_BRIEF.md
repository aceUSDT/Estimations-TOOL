...
- **This is an established multi-tab web app, not a greenfield build.** It is deployed as **"Estimation 101 — Electrical Document Intelligence"** (Netlify SPA, client-side workspace password). It already has the tabs in §2 working: Projects, Documents (with a dropzone + page-classification table), a Viewer (canvas with overlays, assisted canvas, OCR), Devices (count/scope/exclude-spares + CSV/XLSX export), Boards (board hierarchy + "supplied-from" supply relationships + duplicate detection), a Review queue, and Compare (change-type-filtered dual-pane). **Every one of the five workstreams is a change to an existing page — reframe "build" as "modify".** Read the real components before touching them.
...
- **How the new key must be handled (non-negotiable, given this is a client-side SPA):**
  ...
  5. Confirm local dev reads the key from an untracked `.env` (`ANTHROPIC_API_KEY=...`, gitignored) and that Netlify's env var is what production actually uses — report back which of these the current setup already gets right or wrong.
