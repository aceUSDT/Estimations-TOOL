...
| EX-11 | Read schematic board references, traced feed direction and hierarchy, ratings, poles, protective devices, cable construction, meters, SPD, spares, and locations; reconcile only exact board identities and retain selectable source-to-target path provenance. | `test-schematic-topology.mjs`, `verify-schematic-private.mjs`, `verify-real-viewer.mjs` |
