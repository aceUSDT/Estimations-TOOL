...
function parseScheduleLine(line, ctx){
  ...
  const wayM = markerM || compactM || leadM;
  const markerPhase = (markerM && markerM[2]) || (compactM && compactM[2].toUpperCase()) || null;
  ...
  /* A row stating a way and a RATING is a circuit even when it never names the
     ...
     not absent. */
  const compactRating = compactM
    ? (line.slice(compactM[0].length).match(/^\s*(\d{1,4}(?:\.\d+)?)\s*[*\u00b0]?(?=\s|$)/) || [])[1]
    : null;
  ...
  const positional = compactM
    ? (line.slice(compactM[0].length).match(/^\s*(\d{1,4}(?:\.\d+)?)\s*[*\u00b0]?(?=\s|$)/) || [])[1]
    : null;
  const finalRating = positional != null ? Number(positional) : rating;
  const cables2 = cables;
  const row = {
