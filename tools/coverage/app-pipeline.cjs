...
function parseScheduleLine(line, ctx){
  ...
  const compactM = line.match(/^\s*(\d{1,3})(L[123])\b/i);
  ...
  const spacedM = compactM ? null : line.match(/^\s*(\d{1,3})\s+(L[123])\b/i);
  /* All three marker forms open a row the same way: way, phase, then the
     ...
     actually matched. */
  const markerHead = markerM || compactM || spacedM;
  ...
  const markerPhase = (markerM && markerM[2]) || (markerHead && markerHead[2].toUpperCase()) || null;
  ...
  const positionalRating = (() => {
    if (!markerHead) return null;
  ...
  if (!( (wayM && (device||spare||space||rating!=null||compactRating!=null)) || (isIncomer && device) )) return null;
  ...
  const finalRating = positionalRating != null ? Number(positionalRating) : rating;
  const row = {
