...
assert.equal(model.groups[0].rows[0].label, "20A TP MCB");
assert.deepEqual(Array.from(model.groups[0].rows[0].purposes), ["Mechanical"]);
assert.equal(model.groups[1].rows[0].label, "10A SPN Separate RCD 30mA No AFDD RCBO");
assert.deepEqual(Array.from(model.groups[1].rows[0].quantities), [2, 1]);
assert.deepEqual(Array.from(model.groups[1].rows[0].purposes), ["Lighting"]);
assert.equal(model.groups[1].rows[0].curve, "Not specified");
assert.equal(model.groups[1].rows[0].breakingCapacity, "Not specified");
assert.equal(model.groups[1].rows[0].rcdProtected, true);
assert.equal(model.groups[1].rows[0].rcdSensitivity, 30);
assert.equal(model.groups[1].rows[0].protectionStandard, "BS EN 60898");
assert.deepEqual(Array.from(model.boardSections, (board) => board.label), ["DB-2", "DB-10"]);
assert.equal(model.boardSections[0].total, 3);
assert.deepEqual(Array.from(model.boardSections[0].families, (family) => [family.name, family.total]), [["RCBO", 3]]);
...
assert.match(sheet.getCell("C4").value, /RCBO\n10A\nSPN/);
