...
import { normRef as norm } from './lib/metrics.mjs';
...
for (const [name, fn] of Object.entries(engines)) {
  for (const [line, want] of POSITIVE) {
    const got = fn(line);
    if (!got.includes(want)) fail(`[${name}] missing ${want} in ${JSON.stringify(line)} — got [${got}]`);
  }
  for (const line of NEGATIVE) {
    const got = fn(line);
    if (got.length) fail(`[${name}] false positive [${got}] in ${JSON.stringify(line)}`);
  }
  for (const [line, mustNot] of CONTAINMENT) {
    const got = fn(line);
    if (got.includes(mustNot)) fail(`[${name}] kept sub-match ${mustNot} in ${JSON.stringify(line)} — got [${got}]`);
  }
}
finish(`${POSITIVE.length} positives, ${NEGATIVE.length} negatives, containment — both engines.`);
