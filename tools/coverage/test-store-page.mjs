...
/* Playwright is a dev-environment tool, not a repo dependency: resolve it
 * from local node_modules or the global install; skip cleanly when absent so
 * this browser test never breaks a minimal CI. */
let chromium = null;
for (const spec of [resolve(root, 'node_modules/playwright/index.mjs'), 'playwright', 'playwright-core']) {
  try { ({ chromium } = await import(spec)); break; } catch { /* next */ }
}
if (!chromium) { console.log('SKIP (not a failure): playwright not installed — run with NODE_PATH=$(npm root -g).'); process.exit(0); }
